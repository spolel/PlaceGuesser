...
@Component({
  selector: 'app-leaderboard-paged',
  templateUrl: './leaderboard-paged.component.html',
  styleUrls: ['./leaderboard-paged.component.scss'],
})
export class LeaderboardPagedComponent implements OnInit {
  ...
  ngAfterViewInit() {
    ...
    //load first blocks
    this.backend
      .getLeaderboardPaged(0, this.pageSize)
      .pipe
  ...
  getNextBlocks(event: PageEvent) {
    ...
    this.backend
      .getLeaderboardPaged(event.pageIndex, event.pageSize)
      .pipe
      //for each block I am getting the transaction number and adding to to that block
      // mergeMap(blocks => forkJoin(
      //   blocks.map(block =>
